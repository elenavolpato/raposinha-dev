...
export default function FeaturedProjects() {
  ...
  return (
    <div class="min-h-screen text-gray-300 p-8 bg-zinc-900 w-full mt-5 pt-16">
      <div class="max-w-7xl mx-auto">
        ...
        <div class="space-y-32">
          <For each={projects()}>
            {(project) => (
              <div
                class={`flex flex-col ${project.mediaOnRight ? 'lg:flex-row-reverse' : 'lg:flex-row'} gap-8 items-center`}
              >
                {/* Project Image */}
                <div class="lg:w-1/2 relative group">
                  <div class="absolute inset-0 bg-gray-700/20  group-hover:bg-transparent transition-all duration-300"></div>
                  <Show when={!project.video}>
                    <img
                      src={project.media}
                      alt={project.title}
                      class="w-full h-auto shadow-2xl mix-blend-luminosity group-hover:mix-blend-normal transition-all duration-300"
                    />
                  </Show>
                  ...
                  <Show when={project.video}>
                    <div class="relative w-full" style="aspect-ratio: 16/10;">
                      <video
                        ...
                        title={project.title}
                        src={project.media}
                        class="h-full w-full shadow-2xl mix-blend-luminosity group-hover:mix-blend-normal transition-all duration-300"
                      />
                    </div>
                  </Show>
                </div>
                ...
                {/* Project Details */}
                <div
                  class={`lg:w-1/2 ${project.mediaOnRight ? 'lg:text-left' : 'lg:text-right'}`}
                >
                  <div class="space-y-4">
                    ...
                    {/* Description with double border effect */}
                    <div class="relative">
                      <div
                  ...
                  <div
                    class={`flex flex-wrap gap-3 ${project.mediaOnRight ? 'justify-start' : 'justify-end'}`}
                  >
                  ...
                  <div
                    class={`flex gap-4 pt-2 ${project.mediaOnRight ? 'justify-start' : 'justify-end'}`}
                  >
